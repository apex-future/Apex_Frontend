...
const StreakCard = () => {
    ...
    const { streakCount, streakHistory, frozenDays = [] } = useStudyStore();
            ...
            >
                ...
                <div className="flex justify-between w-full max-w-xs md:max-w-md gap-1 md:gap-2 relative z-10">
                    {days.map((day, index) => {
                        ...
                        const isFrozen = frozenDays.includes(dayDate);
                        const hasStreak = streakHistory.includes(dayDate);

                        return (
                            <div key={day} className="flex flex-col items-center gap-1.5 md:gap-2">
                                <span className={`text-[10px] md:text-xs font-bold uppercase tracking-wider ${index === currentDay ? (isFrozen ? 'text-sky-500' : 'text-orange-500') : 'text-text-tertiary'}`}>
                                    {day}
                                </span>
